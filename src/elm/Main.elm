...
type Msg
    = New2dPoint (Result String Point2d)
...
update msg model =
    case msg of
        New2dPoint result ->
            ( { model | point2d = result |> Result.unpack identity showPoint2d |> Just }, Cmd.none )

        New3dPoint result ->
            ( { model | point3d = result |> Result.unpack identity showPoint3d |> Just }, Cmd.none )

        GenericEvent text ->
            ( { model | generic = Just text }, Cmd.none )
...
{- Silently drop events we don't know about. Can't happen anyway. -}
